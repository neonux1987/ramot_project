...
class SetupLogic {
  ...
  async firstTimeSetup() {
    try {
      ...
      if (setupConfigFile.firstTime) {
        ...
        await fse.copy(path.join(SystemPaths.paths.setup_folder_path, "db", SystemPaths.info.db_file_name), `${SystemPaths.paths.db_file_path}`, COPY_OPTIONS);
  ...
  async setLocations() {
    ...
    config.locations.services_file_path = SystemPaths.paths.services_path;
    config.locations.backups_names_file_path = SystemPaths.paths.backups_names_file_path;

    config.location.logs_folder = SystemPaths.paths.logs_folder;
    config.location.log_file = SystemPaths.paths.log_file;
