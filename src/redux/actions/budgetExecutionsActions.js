...
export const TYPES = {
  ...
  BUDGET_EXECUTIONS_REMOVE_BUILDING_STATE:
    "BUDGET_EXECUTIONS_REMOVE_BUILDING_STATE",
...
export const deleteBudgetExecution = (
  buildingId,
  date,
  index,
  rollbackData
  ...
  return (dispatch) => {
    ...
    return ipcSendReceive({
      ...
      onError: (result) => {
        dispatch(
          addBudgetExecutionInStore(buildingId, rollbackDataCopy, sortByCode)
        );
      }
    });
  };
  ...
  return (dispatch) => {
    return ipcSendReceive({
      ...
      onSuccess: (result) => {
        ...
        dispatch(
          addBudgetExecutionInStore(buildingId, result.data, sortByCode)
        );
...
export const updateBudgetExecution = (
  params = Object,
  oldBudgetExec = Object,
  newBudgetExec = Object,
  index = Number
) => {
  return (dispatch, getState) => {
    ...
    const monthlyStatsArr = [
      ...state.monthlyStats[buildingId].pages[pageName].data
    ];
    ...
    const quarterlyStatsData = {
      ...state.quarterlyStats[buildingId].pages[pageName].data[0]
    };
    ...
    if (params.date.month) {
      ...
      monthStatsObject.income =
        monthStatsObject.income -
        oldBudgetExec[`${params.date.month}_budget`] +
        newBudgetExec[`${params.date.month}_budget`];

      //subtract the old quarter budget value from
      //total budget and then add the new quarter budget value
      quarterlyStatsData.income =
        quarterlyStatsData.income -
        oldBudgetExec["total_budget"] +
        newBudgetExec["total_budget"];

      //update month total
      dispatch(
        monthlyStatsActions.updateMonthStatsStoreOnly(
          buildingId,
          pageName,
          monthStatsObject,
          monthStatsIndex
      ...
      dispatch(
        quarterlyStatsActions.updateQuarterStatsStoreOnly(
          buildingId,
          pageName,
          quarterlyStatsData
        )
    ...
    dispatch(
      updateBudgetExecutionStoreOnly(
        budgetExecStoreObj,
        index,
        params.buildingId
      )
    ...
    return ipcSendReceive({
      ...
      onError: () => {
        ...
        dispatch(
          quarterlyStatsActions.updateQuarterStatsStoreOnly(quarterStatsOld)
        );
      }
    });
  };
