...
const ExcelReportsGenerator = ({
  ...
  onYearChangeHandler,
  noDataText = "לא קיימים דוחות בבסיס נתונים לפחות לאחד מהבניינים ולכן לא ניתן להפיק דוחות אקסל"
}) => {
  const content = () => {
    return (
      <Fragment>
        <div className={selectsWrapper}>
          ...
          <WhiteButton onClick={onClickHandler}>צור</WhiteButton>
        </div>
  ...
  const renderContent =
    registeredReports.data.length === 0 ? (
      <span className={error}>{noDataText}</span>
    ) : (
      content()
    );
