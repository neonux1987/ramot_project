...
const PageControls = ({ excel, print, pageName, ...props }) => {
  const exportToExcel = () => {
    saveToFile(excel.fileName, (fullPath) => {
      if (fullPath)
        excelProcess(fullPath, excel.fileName, pageName, excel.data);
    });
  }

  return (
    <div id="page-controls" className={styles && styles.pageControls ? styles.pageControls : props.classes.pageControls}>
      <button className={styles && styles.pageControlsButton ? styles.pageControlsButton : props.classes.pageControlsButton} onClick={excel.handler ? excel.handler : exportToExcel}>
      ...
      <button className={styles && styles.pageControlsButton ? styles.pageControlsButton : props.classes.pageControlsButton} onClick={() => printProcess({
