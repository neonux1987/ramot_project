...
const Menuitem = (props) => {
  ...
  let subMenuItems = props.item.submenu.map((item, index) => {
    return (<NavButton
      buildingName={props.item.label}
      buildingNameEng={props.item.engLabel}
      page={item.label} path={props.item.path + "/" + item.path}
      active={props.active.subMenuItemId === item.id}
