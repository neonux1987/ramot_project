...
export const generateEmptyReports = (params) => {
  return ipcSendReceive({
    ...
    onSuccess: ({ data }) => {
      const { existingReportsBuidlings, createdReportsBuidlings } = data;

      if (existingReportsBuidlings.length > 0)
        toastManager.warning(
          `לבניינים '${existingReportsBuidlings.toString()}' לא נוצרו דוחות לתאריך הנבחר מאחר וקיימים כבר במערכת`
        );
