...
class SummarizedBudgetLogic {
  ...
  getBuildingSummarizedBudgetTrx(buildingId, date, trx) {
    return this.summarizedBudgetDao.getBuildingSummarizedBudgetTrx(
      buildingId,
      date,
      trx
    );
  ...
  async getSummarizedBudgetsByRange(buildingId, fromYear, toYear) {
    return this.summarizedBudgetDao.getSummarizedBudgetsByRange(
      buildingId,
      fromYear,
      toYear
    );
  ...
  getSummarizedBudgetTopIncomeOutcome(buildingId, fromYear, toYear, limit) {
    return this.summarizedBudgetDao.getSummarizedBudgetTopIncomeOutcome(
      buildingId,
      fromYear,
      toYear,
      limit
  ...
  getSummarizedBudgetByIdTrx(summarized_section_id, buildingId, date, trx) {
    return this.summarizedBudgetDao.getSummarizedBudgetByIdTrx(
  ...
  async updateSummarizedBudgetTrx(
    {
      summarized_section_id,
      summarizedBudget = Object,
      buildingId = String,
      date = Object,
      special = false
    },
    ...
    await this.summarizedBudgetDao.updateSummarizedBudgetTrx(
      summarized_section_id,
      buildingId,
      summarizedBudget,
      date,
    ...
    const allSummarizedBudgets =
      await this.summarizedBudgetDao.getBuildingSummarizedBudgetTrx(
        buildingId,
        date,
        trx
      );
    ...
    if (!special) {
      ...
      //update year stats
      await this.yearlyStatsLogic.updateYearStatsTrx(
        buildingId,
        date,
        {
  ...
  prepareYearStats(allSummarizedBudgets) {
    ...
    for (let i = 0; i < allSummarizedBudgets.length; i++) {
      if (
        allSummarizedBudgets[i].section !== "הכנסות מיוחדות" ||
        allSummarizedBudgets[i].summarized_section_id !== 31
      ) {
  ...
  async createEmptyReport(buildingId, date, fromPreviousReports, trx) {
    const registeredYear = await this.registeredYearsLogic.getRegisteredYearTrx(
      buildingId,
      date.year,
      trx
    );
    ...
    const sumBudgets = await this.getBuildingSummarizedBudgetTrx(
      buildingId,
      newDate,
      trx
    );
    ...
    if (fromPreviousReports && sumBudgets.length > 0) {
      ...
      await this.summarizedBudgetDao.batchInsert(
        buildingId,
    ...
    await this.quarterlyStatsLogic.batchInsert(
      buildingId,
      quarterlyStatsArr,
      trx
    );
    ...
    const result = await this.yearlyStatsLogic.insertYearStatsTrx(
      buildingId,
      {
        year: date.year,
        income: 0,
    ...
    await this.registeredYearsLogic.registerNewYear(
      buildingId,
      { year: date.year },
      trx
    );
