...
const yearlyStatsIpc = (connection) => {
  ...
  ipcMain.on('get-year-stats', (event, arg) => {
    yearlyStatsLogic.getYearStatsTrx(arg.buildingId, arg.date).then((result) => {
  ...
  ipcMain.on('get-yearly-stats-by-year-range', (event, { buildingId, fromYear, toYear }) => {
    yearlyStatsLogic.getYearStatsByYearRange(buildingId, fromYear, toYear).then((result) => {
