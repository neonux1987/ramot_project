...
const updatesIpc = () => {
  ...
  ipcMain.on("check-for-updates", async (event) => {
    ...
    if (result === null) {
      event.sender.send("checked_for_updates", {
        error: "בדיקת עידכונים חדשים נכשלה"
      });
    } else {
      ...
      else event.sender.send("checked_for_updates", { data: null });
    }
  });
  ...
  ipcMain.on("abort-download", (event) => {
    ...
    if (cancellationToken === undefined || cancellationToken._cancelled)
      ...
      sendToWindow("download_aborted", {
        error: "המערכת לא הצליחה לבטל את ההורדה"
      });
  });
  ...
  ipcMain.on("delete-update", (event, path) => {
    deleteUpdate(path)
      ...
      .catch((error) => {
        console.log(error);
        sendToWindow("update_deleted", {
          error:
            "המערכת לא הצליחה למחוק את העידכון. אתחל את האפליקציה ולאחר מכן נסה שנית."
        });
      });
  });
